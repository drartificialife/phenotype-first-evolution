...
GENE-CENTRIC GA
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Initialize population P of size 50
For each generation (150 generations):
  For each organism O in P:
    phenotype = genome  # Direct mapping
    fitness = evaluate(phenotype, target)

  # Selection (tournament)
  parents = select_tournament(P, fitness)

  # Reproduction
  offspring = []
  For each parent in parents:
    genome_offspring = mutate(parent.genome, mutation_rate=0.02)
    offspring.append(Organism(genome_offspring))

  P = offspring  # Replace population

Return best organism found
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Key point: NO LEARNING. Phenotype determined by genes only.
\end{verbatim}
...
For each generation (150 generations):
  For each organism O in P:
    learning_layer = zeros(100)  # Start empty

    # Learning phase: 20 trials of hill-climbing
    For trial = 1 to 20:
      candidate = learning_layer.copy()
      candidate[random_bit] = flip(candidate[random_bit])
      phenotype = O.genome ∪ candidate
      fitness_new = evaluate(phenotype, target)

      if fitness_new > evaluate(O.genome ∪ learning_layer, target):
        learning_layer = candidate  # Keep improvement

    phenotype = O.genome ∪ learning_layer
    fitness = evaluate(phenotype, target)

  # Selection based on phenotype fitness
  parents = select_tournament(P, fitness)

  # Reproduction: inherit genome only, NOT learned layer
  offspring = []
  For each parent in parents:
    genome_offspring = mutate(parent.genome, mutation_rate=0.02)
    offspring.append(Organism(genome_offspring))
    # Note: learning_layer resets to empty for offspring

  P = offspring

Return best organism found
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Key point: Learning improves survival, but offspring don't inherit learned patterns.
Knowledge accumulates genetically over many generations (slow).
\end{verbatim}
...
\textbf{Pseudocode:}
\begin{verbatim}
PHENOPOIESIS (Phenotype-First with Epigenomic Inheritance)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Initialize population P of size 50
For each generation (150 generations):
  For each organism O in P:
    epigenome = O.epigenome.copy()  # Start with inherited pattern

    # Learning phase: 20 trials modifying epigenome only
    For trial = 1 to 20:
      candidate = epigenome.copy()
      candidate[random_bit] = flip(candidate[random_bit])
      phenotype = O.genome ∪ candidate
      fitness_new = evaluate(phenotype, target)

      if fitness_new > evaluate(O.genome ∪ epigenome, target):
        epigenome = candidate  # Keep improvement

    phenotype = O.genome ∪ epigenome
    fitness = evaluate(phenotype, target)

  # Selection based on phenotype fitness
  parents = select_tournament(P, fitness)

  # Reproduction: inherit BOTH genome AND epigenome
  offspring = []
  For each parent in parents:
    # Genome: mutated
    genome_offspring = mutate(parent.genome, mutation_rate=0.02)

    # Epigenome: PERFECT COPY (100% fidelity)
    epigenome_offspring = parent.epigenome.copy()  # No mutations

    offspring.append(Organism(genome_offspring, epigenome_offspring))

  P = offspring

Return best organism found
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Key point: CRITICAL DIFFERENCE from Baldwin Effect:
  - Offspring inherit parent's learned epigenomic patterns at 100% fidelity
  - Offspring don't need to rediscover parent's solutions
  - Knowledge compounds: each generation adds new learning on top of inherited learning
  - This enables 3-6× faster learning on compatible compositional tasks
\end{verbatim}
